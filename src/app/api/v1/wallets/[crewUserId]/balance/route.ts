import { getWalletBalance } from '@/modules/payments/actions/wallet-actions';
...
export const GET = withAuth(async (_request, context, routeContext) => {
  ...
  const balance = await getWalletBalance(context, crewUserId);
  return ok(balance, undefined, { requestId: context.requestId });
});
