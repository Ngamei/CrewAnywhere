import { getWallet } from '@/modules/payments/actions/wallet-actions';
...
export const GET = withAuth(async (_request, context, routeContext) => {
  ...
  const wallet = await getWallet(context, crewUserId);
  return ok(wallet, undefined, { requestId: context.requestId });
});
